...
final class ContactsManager {
    ...
    var loadedHandler: ((Bool) -> Void) = { _ in }
    ...
    func setFavorite(forContact contact: Contact) {
        contact.isFavorite = !contact.isFavorite
        Storage.save(appContacts)
    }

    func setNames(forContact contact: Contact, fromFullName fullName: String) {
        let names = fullName.split(separator: " ", maxSplits: 2).map { String($0) }

        switch names.count {
        case 0:
            updateNames(forContact: contact, givenName: ":]", middleName: "", familyName: "")
        case 1:
            updateNames(forContact: contact, givenName: names[0], middleName: "", familyName: "")
        case 2:
            updateNames(forContact: contact, givenName: names[1], middleName: "", familyName: names[0])
        default:
            updateNames(forContact: contact, givenName: names[1], middleName: names[2], familyName: names[0])
        }
    }
    ...
    func setPhoneFormat(forContact contact: Contact, fromNumber number: String) {
    ...
    private func maskFormat(forNumber number: String) -> String? {
        ...
        default:
            return PhoneMask(rawValue: number.count)?.mask
        }
    }
    ...
    private enum PhoneMask: Int {
        ...
        var mask: String {
            ...
            case .tenNumbersWithFirst0:
                return "+375 (DXX) XXX XX XX"
            case .tenNumbersWithFirst8:
                return "X0XX-XXX-XX-XX"
            case .elevenNumbers:
                return "XXXX-XXX-XX-XX"
            case .twelveNumbers:
